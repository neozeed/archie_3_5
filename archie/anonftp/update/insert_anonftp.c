...
extern status_t insert_compare_hstart_db(full_site_entry_t *fp1, int s1,
                                         full_site_entry_t *fp2, int s2,
                                         file_info_t *start_db, header_t header_rec,
                                         int port);
extern status_t insert_hstart_db(full_site_entry_t *fp, file_info_t *start_db,
                                 file_info_t *domain_db, header_t header_rec,
                                 int port);
...
#define BUFF_SIZE 8192
   ...
   extern status_t insert_hstart_db( full_site_entry_t *,
                                     file_info_t *, file_info_t *,
                                     header_t, int );
   extern status_t insert_compare_hstart_db( full_site_entry_t *,
                                             int,  full_site_entry_t *,
                                             int,  file_info_t *, header_t,
                                             int );
   extern status_t count_extra_recs( char *, int, header_t * );
#else
...
/*
 * This is the main controlling routine for the insert_anonftp program.
 * This program inserts information given in the input file into the archie
 * anonftp database. It modifies the hostaux_db with the new information
...

{
  ...
  while((option = getopt(argc, argv, "i:w:vt:I:h:M:sL:l")) != EOF){
    ...
    default:
      error(A_ERR,"insert_anonftp","Unknown option '%s'\nUsage: insert_anonftp  -i <input> -w <anonftp files database pathname>  -M <master database pathname> -T <start database pathname>  -h <host database pathname> -t <temporary directory pathname> -I <minimum size for index> -v -s -l -L <log file>", *cmdline_ptr);
      exit(A_OK);
  ...
  if(logging){
    if(logfile[0] == '\0'){
      if(open_alog((char *) NULL, A_INFO, tail(argv[0])) == ERROR){
        ...
        error(A_ERR, "insert_anonftp", INSERT_ANONFTP_020);
        exit(A_OK);
      }
    }
    else{
      if(open_alog(logfile, A_INFO, tail(argv[0])) == ERROR){
        ...
        error(A_ERR, "insert_anonftp", INSERT_ANONFTP_019, logfile);
        exit(A_OK);
      }
    }
  }
  ...
  if(input_finfo -> filename[0] == '\0'){
    ...
    error(A_ERR,"insert_anonftp", INSERT_ANONFTP_024);
    exit(A_OK);
  }
  ...
  if(set_master_db_dir(master_database_dir) == (char *) NULL){
    ...
    error(A_ERR, "insert_anonftp", INSERT_ANONFTP_022);
    exit(A_OK);
  }
  ...
  if(set_host_db_dir(host_database_dir) == (char *) NULL){
    ...
    error(A_ERR, "insert_anonftp", INSERT_ANONFTP_023);
    exit(A_OK);
  }
  ...
  if(open_file(input_finfo, O_RDONLY) == ERROR){
    ...
    error(A_SYSERR,"insert_anonftp", INSERT_ANONFTP_001, input_finfo -> filename);
    exit(A_OK);
  }
  ...
  if(read_header(input_finfo -> fp_or_dbm.fp, &header_rec, &offset, 0, 0) == ERROR){
    ...
    error(A_ERR,"insert_anonftp", INSERT_ANONFTP_002, input_finfo -> filename);
    exit(A_OK);
  }
  ...
  if((int)(input_finfo -> size - offset) <= 0){
    ...
    error(A_ERR, "insert_anonftp", INSERT_ANONFTP_026, input_finfo -> filename);
    exit(A_OK);
  }
  ...
  if( count_extra_recs( input_finfo->ptr, input_finfo->size, &header_rec) == ERROR){
    ...
    error(A_WARN, "insert_anonftp","Can't count extra records to be added to output file %s", output_finfo -> filename);
    goto unlink_output;
  }
...
/*  fprintf(stderr,"records after recountig :no_recs=%li  site_no_recs=%li\n",header_rec.no_recs,header_rec.site_no_recs); */
  ...
  if(setup_output_file(output_finfo, &header_rec, tmp_dir) == ERROR){
    ...
    error(A_WARN, "insert_anonftp", INSERT_ANONFTP_017, output_finfo -> filename);
    goto unlink_output;
  }
  ...
  if ( open_start_dbs(start_db,domain_db,O_RDWR ) == ERROR ) {
    ...
    /* "Can't open stbart/host database" */

    error(A_ERR, "insert_anonftp", "Can't open start/host database");
    goto unlink_output;
  }

  if ( !(strhan = archNewStrIdx()) ) {
    error( A_ERR, "insert_anonftp", "Could not find create string handler");
    exit(A_OK);
  }
  ...
  if ( archStridxExists( dbdir, &ans) ){
    ...
    case 1:                     /* all the files exist */
      if ( !archOpenStrIdx( strhan, dbdir, ARCH_STRIDX_APPEND ) ) {
        error( A_ERR, "insert_anonftp", "All the Strings files exist but could not be open! Check you permissions. Exiting");
        exit(ERROR);
      }
      break;

    case -1:                    /* some files exist */
      error( A_ERR, "insert_anonftp", "Some of the Strings files exist but not all of them. Couldn't open Strings files. Exiting");
      exit(ERROR);
      break;

    case 0:                     /* no strings files exist. */
      error( A_INFO, "insert_anonftp", "Could not find Strings files, will create them." );
      first = 1;

      if ( !(strhan = archNewStrIdx()) ) {
        error( A_ERR, "insert_anonftp", "Could not create a string handler");
        exit(A_OK);
      }
    
      if ( !archCreateStrIdx( strhan, dbdir, ARCH_INDEX_CHARS ) ) {
        error( A_ERR, "insert_anonftp", "Could not create strings_idx files." );
        exit(A_OK);
      }
      break;
    ...
    error( A_ERR, "insert_anonftp", "archStridxExists() failed. Something is terribly wrong! Report to archie-group@bunyip.com");
    exit(ERROR);
  }
  ...
  if(open_host_dbs(hostbyaddr, hostdb, (file_info_t *) NULL, hostaux_db, O_RDWR) != A_OK){
    ...
    error(A_ERR, "insert_anonftp", NOT_OPEN_MASTER_000);
    goto unlink_output;
  }

  /* "Doing hostname lookup" INSERT_ANONFTP_006 */

  if(verbose)
  error(A_INFO,"insert_anonftp","Inactivating the site if found in the database. ");

  if(inactivate_if_found(header_rec.primary_hostname, ANONFTP_DB_NAME, header_rec.preferred_hostname, header_rec.access_command, hostaux_db) == ERROR){

    /* "Error while trying to inactivate %s in host database" */

    error(A_ERR,"delete_anonftp", DELETE_ANONFTP_009, header_rec.primary_hostname);
    goto unlink_output;
  }

  if((host_st = do_hostdb_update(hostbyaddr, hostdb, hostaux_db, &header_rec, ANONFTP_DB_NAME, flags, 0)) != HOST_OK){
    ...
    error(A_ERR,"insert_anonftp", INSERT_ANONFTP_007, get_host_error(host_st));
    goto unlink_output;
  }
  ...
  if(header_rec.update_status == FAIL){
    ...
    if(unlink(output_finfo -> filename) == -1){
      ...
      error(A_SYSERR, "insert_anonftp", INSERT_ANONFTP_025, output_finfo -> filename);
      exit(ERROR);
    }
  ...
  if(verbose)
  error(A_INFO,"insert_anonftp", INSERT_ANONFTP_009);

  if(setup_insert(strhan, input_finfo -> ptr, input_finfo -> size,
                  (full_site_entry_t *) output_finfo -> ptr,
                  &output_rec_no, header_rec) == ERROR){

    error(A_ERR, "insert_anonftp", INSERT_ANONFTP_010);
    goto unlink_output;
  }

  /* just like we used open_output_file() using the header_rec->site_no_recs
     to allocate memory, we must use a remap_output_file() to reallocate
     the memory with the new (definite) site_no_recs */


  if(activate_site(&header_rec, ANONFTP_DB_NAME, hostaux_db) == ERROR){

    /* "Can't activate host database record for %s" */

    error(A_ERR,"insert_anonftp", INSERT_ANONFTP_016, header_rec.primary_hostname);
    exit(A_OK);
  }
  ...
  if( get_port( header_rec.access_command, ANONFTP_DB_NAME, &port ) == ERROR ){
    error(A_ERR,"insert_anonftp","Could not find the port for this site %s", header_rec.primary_hostname);
    exit(A_OK);
  }
  
  if(access(files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port), F_OK) < 0){

    /* "File %s does not exist in the database. No need to compare" */

    error(A_INFO,"insert_anonftp", "File %s does not exist in the database. No need to compare",
          files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port));
    ...
    {
      error(A_ERR,"insert_anonftp","can't insert into start/host database site %s",
            files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port));
      goto unlink_output;
    }
  }
  else
  {
    ...
    strcpy(curr_finfo -> filename, files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port));
                                 ...
                                 curr_size, start_db, header_rec, port ) == ERROR )
    {
      ...
      error(A_ERR,"insert_anonftp","Error in compare_site_files for site %s." ,
            files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port));
      goto unlink_output;
    }
  }
  ...
  if(rename(output_finfo -> filename, files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port)) != 0){

    if ( errno != EXDEV || 
        archie_rename(output_finfo -> filename,
                    files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port)) != 0){

	    /* "Can't rename %s to %s. anonftp database not changed." */
	    
	    error(A_SYSERR,"insert_anonftp", INSERT_ANONFTP_013, output_finfo -> filename,
            files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port));
	    goto unlink_output;
    }
  }
  ...
  sprintf(file2,"%s%s",files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port),SITE_INDEX_SUFFIX);
  ...
  if ( index_file ) {
    ...
    if(rename(file1, file2) != 0 ) {
      if ( errno != EXDEV || 
        ...
        error(A_SYSERR,"insert_anonftp", INSERT_ANONFTP_013, file1, file2);
        goto unlink_output;
      }
    }
  }else{
  ...
  strcpy(output_finfo->filename,files_db_filename(inet_ntoa( ipaddr_to_inet(header_rec.primary_ipaddr)),port));
  if ( first ){ 
    if ( ! archUpdateStrIdx(strhan)) {
      error( A_ERR, "insert_anonftp", "Could not update strings_idx files.");
 ...
 unlink_output:
  ...
  /*
     if ( (strhan) && ! archUpdateStrIdx(strhan)) {
     error( A_WARN, "insert_anonftp", "Could not update strings_idx files.");
     archCloseStrIdx(&strhan);
     exit(A_OK);
     }
     */
   ...
   char *tmp_dir;	   /* pathname of temporary directory */
{
  ...
  if((int) ftruncate(fileno(output_finfo -> fp_or_dbm.fp),
                     (header_rec->site_no_recs )*sizeof(full_site_entry_t)) != 0){
   ...
   int *arsz;
{
  ...
  for ( no=0; no<sz ; no++ )
  {
    ...
    {
      if( i >= *arsz )
      {   error( A_ERR,"insert_anonftp","failed in create_str_array");
          return(ERROR);
        }
  ...
  int port;
{
  ...
  if ( host_table_find(&ip,
                       header_rec.primary_hostname, &port, &hin ) == ERROR )
  {
    /* "New site %s will be added to host/start table\n" */
    error(A_INFO,"insert_anonftp","New site %s will be added to host/start table",
    ...
    {                           /* "insert_hstart_db : Could not add host %s to host_table" */
      error(A_ERR,"insert_anonftp","insert_hstart_db : Could not add host %s to host_table",
            (char *)(inet_ntoa( ipaddr_to_inet(ip))) );
      return ERROR;
    }
  ...
  for ( no=0; no<sz ; no++ )
  {
        ...
        CSE_IS_KEY(fp[no]))
    {
      ...
      {                         /* "Could not update start/host table with  %s ." */
        error(A_ERR,"insert_anonftp","Could not update start/host table with  %s .",
              (char *)(inet_ntoa( ipaddr_to_inet(ip))) );
        return ERROR;
      }
    }
  ...
  error(A_INFO,"insert_anonftp","Successful update in start/host dbase with %s .",
        (char *)(inet_ntoa( ipaddr_to_inet(ip))) );
  ...
  int port;
{
  ...
  ip = header_rec.primary_ipaddr;
  error(A_INFO,"insert_anonftp","Starting insert_compare_hstart_db on %s",
        (char *)(inet_ntoa( ipaddr_to_inet(ip))) );
  ...
  error(A_INFO,"insert_anonftp","Finished insert_compare_hstart_db on %s",
        (char *)(inet_ntoa( ipaddr_to_inet(ip))) );
